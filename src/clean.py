...
def filter_csv(folder, file, chunk_size=1000, universidade='ufob', cod_orgao='26447', cod_uorg='26232000000732'):
    filtered_data = []
    with open(folder + file, 'r', encoding='latin1') as f:
        ...
        for i, row in enumerate(reader):
            if (row[17] == cod_orgao or str(row[23]) == cod_orgao) and row[15] == cod_uorg:
                    chunk.append(row)
            if (i + 1) % chunk_size == 0:
    ...
    new_file = os.path.join(filtred_folder,  file.replace('.csv', f'_{universidade}.csv'))
...
def concatenate_csv_files(folder_path, output_file, universidade):
    ano = output_file[:4]
    mes = output_file[5:7]
    ...
    with open(output_file, 'w', encoding='latin1') as f:
        ...
        for csv_file in csv_files:
            if csv_file.endswith(f'_{universidade}.csv'):
